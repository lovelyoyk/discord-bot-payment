...
class PaymentCog(commands.Cog):
    ...
    @app_commands.describe(
        ...
        emoji="Novo emoji (ex: :check_yes: ou <a:check_yes:ID>)"
    )
    async def config_emojis(self, interaction: discord.Interaction, tipo: str = None, emoji: str = None):
        """Configura os emojis usados nas notificações.
        
        Uso: /config_emojis sucesso :meu_emoji:
        
        Tipos disponíveis:
        - sucesso (✅ quando pagamento aprovado)
        - cliente (👥 para identificar cliente)
        - vendedor (👤 para identificar vendedor)
        - valor (💰 para valores em reais)
        - pagamento (💳 para transações)
        ...
        # Se não informar tipo, mostrar configuração atual
        if tipo is None or emoji is None:
            embed = discord.Embed(
                title="🎨 Configuração de Emojis",
                description="Emojis usados nas notificações de pagamento\n\nUse: `/config_emojis <tipo> <emoji>`",
                color=discord.Color.blue()
            )
            
            embed.add_field(
                name="✅ SUCESSO",
                value=f"{EMOJI_SUCESSO} `{EMOJI_SUCESSO}`",
                inline=True
            )
            
            embed.add_field(
                name="👥 CLIENTE",
                value=f"{EMOJI_CLIENTE} `{EMOJI_CLIENTE}`",
                inline=True
            )
            
            embed.add_field(
                name="👤 VENDEDOR",
                value=f"{EMOJI_VENDEDOR} `{EMOJI_VENDEDOR}`",
                inline=True
            )
            
            embed.add_field(
                name="💰 VALOR",
                value=f"{EMOJI_VALOR} `{EMOJI_VALOR}`",
                inline=True
            )
            
            embed.add_field(
                name="💳 PAGAMENTO",
                value=f"{EMOJI_PAGAMENTO} `{EMOJI_PAGAMENTO}`",
                inline=True
            )
            
            embed.add_field(
                name="📝 Exemplos de Uso",
                value="`/config_emojis sucesso 🎉`\n`/config_emojis cliente 👨`\n`/config_emojis valor 💵`",
                inline=False
            )
        ...
        tipo = tipo.lower()
        tipos_validos = {
            "sucesso": "EMOJI_SUCESSO",
            "cliente": "EMOJI_CLIENTE",
            "vendedor": "EMOJI_VENDEDOR",
            "valor": "EMOJI_VALOR",
            "pagamento": "EMOJI_PAGAMENTO"
        }
        
        if tipo not in tipos_validos:
            embed = discord.Embed(
                title="❌ Tipo Inválido",
                description=f"Tipos válidos: {', '.join(tipos_validos.keys())}",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        try:
            ...
            env_var = tipos_validos[tipo]
            old_line = None
            new_line = f"{env_var}={emoji}"
            ...
            embed = discord.Embed(
                title="✅ Emoji Configurado",
                description=f"**{tipos_validos[tipo]}** foi atualizado para:\n\n{emoji}\n\n⚠️ O bot será reiniciado para aplicar a mudança.",
                color=discord.Color.green()
            )
            ...
            embed = discord.Embed(
                title="❌ Erro",
                description=f"Não foi possível atualizar o emoji: {str(e)}",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        
        embed.set_footer(text="Para alterar, edite o arquivo .env e reinicie o bot")
